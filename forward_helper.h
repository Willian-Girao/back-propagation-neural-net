...
void network_forward(datapoint *cur_datapoint, hidden_layers *hidden_layers_set, neuron* output_neuron) {
	hidden_layers *hidden_set = hidden_layers_set;
	hidden_layer *aux_last_layer = NULL;
	...
	while (hidden_set) {
		...
		aux_last_layer = hidden_set->layer_k; /* Saves reference to last processed layer */
		hidden_set = hidden_set->next_layer;
	}

	operate_neuron(output_neuron, layer_neurons_outputs(aux_last_layer)); /* Gets the outputs of last hidden layer to compute the network's output */
}